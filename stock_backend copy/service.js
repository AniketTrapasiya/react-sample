...
async function data(req, res) {
    ...
    try {
        const response = await axios.post('http://18.119.106.158/predict/predict', {
            "SYMB": req.body.SYMB,
...
async function pattern(req, res) {
    ...
    // const url = `https://query1.finance.yahoo.com/v8/finance/chart/${req.body.SYMB}?region=US&lang=en-US&includePrePost=false&interval=${req.body.Interval}m&useYfid=true&range=${req.body.Range}d&corsDomain=finance.yahoo.com&.tsrc=finance`;
    try {
        const response = await axios.post('http://18.119.106.158/predict/pattern', {
            "SYMB": req.body.SYMB,
            "Interval": req.body.Interval,
            "Range": req.body.Range
        }
        ...
        if (response.status === 200) {
            ...
            // logger.info(JSON.stringify(data))
            if (data[0]) {
                const value = await data.Value.map(item =>
                    [
                        moment(item[0]).unix() * 1000,
                        parseFloat(item[1], 10),
                    ],
                );
...
module.exports = { data, fetchData, vwap, sma, pattern }